...
if ( isset($_REQUEST['cImage']) ) {
    $image_id = $_REQUEST['cImage'];
}

if ($select_image_data = $database->prepare('SELECT path,username,created,format FROM images WHERE id=?')) {
    $select_image_data->bind_param('i', $image_id);
    $select_image_data->execute();
    $select_image_data->bind_result($iPath, $iUsername, $iCreated, $iFormat);
    $select_image_data->fetch();
    $select_image_data->close();
}

$cType = "i";
if ($image_comment = $database->prepare('SELECT username,comment,created FROM comments WHERE element_id=? AND type=? ORDER BY created DESC')) {
    $image_comment->bind_param("is", $image_id, $cType);
...
if ($count_likes = $database->prepare('SELECT COUNT(*) FROM likes WHERE element_id=? AND type=?')) {
    $count_likes->bind_param('ss', $image_id, $cType);
    $count_likes->execute();
...
if ( $cLiked == 0 ) {
    $cLiked = "Sei der/die Erste, dem <b style='cursor: pointer;' onclick=\"Like('${image_id}', 'i', '0', '1');\">das gefällt</b>";
} else {
    $cLiked = "<b style='cursor: pointer;' onclick=\"ToggleUserModal('${cUsername}', '${image_id}', 'i', '1');\">Gefällt " . $cLiked . " Mal</b>";
}
...
echo <<<EOF
        <div class="modal-content">
            ...
            <div class="row">
                    ...
                    <div class="s_modal_text_container" id="action_element_information_${cType}_${image_id}">
                        <div class="s_modal_creator"> 
                            <div class="row">
                                <div class="row col-sm-2 center-sm middle-sm">
                                
                                    <div class="h_round_image" style="background-image: url(${_SESSION['path']});"></div>
...
EOF;
                        ...
                        foreach ($Comments as $Comment):
                            ...
                            $uPath = GetProfilePicture( $Comment["username"], $database );
...
EOF;
                                ...
                                if ($current_user_like = $database->prepare('SELECT COUNT(*) FROM likes WHERE element_id=? AND type=? AND username=?')) {
                                    $current_user_like->bind_param('sss', $image_id, $cType, $_SESSION['name']);
                                ...
                                if ( $currentLike == 0 ) {
                                    echo "<div class=\"col-sm-2\"><button onclick=\"Like('${image_id}', 'i', '0');\"><i class=\"far fa-heart\"></i></i></button></div>";
                                } else {
                                    echo "<div class=\"col-sm-2\"><button onclick=\"Like('${image_id}', 'i', '1');\"><i class=\"far fa-heart-broken\"></i></i></button></div>";
                                }
                    ...
                    <div class="s_modal_input">
                        <textarea id="action_comment_textarea_i_${image_id}" placeholder="Kommentar hinzufügen"></textarea>
                        <button onclick="WriteComment( '${image_id}', 'i', '1' );">Posten</button>
                    </div>
                </div>
            </div>
          </div>
