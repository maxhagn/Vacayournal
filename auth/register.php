...
// Try Save
if ($can_save == TRUE) {
    if ($check_user = $database->prepare('SELECT id, password FROM users WHERE email=?')) {
        $check_user->bind_param('s', $cEmail);
        $check_user->execute();
        ...
        if ($user_exist > 0) {
            $_SESSION['register_error'][16] = "Die E-Mail-Adresse wurde bereits registriert, hast du deine Anmeldedaten vergessen, kannst du das Passwort zurücksetzen.";
        } else {

            $cQuery = "";
            $cInsertValidation = "";
            if ( $cVerifyBy == 1 ) { $cQuery = "INSERT INTO users (first_name, last_name, password, email, created, changed, verified, birth_date, gender) VALUES (?, ?, ?, ?, NOW(), NOW(), ?, ?, ?)"; $cInsertValidation = $cEmail; }
            elseif ( $cVerifyBy == 2 ) { $cQuery = "INSERT INTO users (first_name, last_name, password, mobile, created, changed, verified, birth_date, gender) VALUES (?, ?, ?, ?, NOW(), NOW(), ?, ?, ?)"; $cInsertValidation = $cPhone; }

            if ($insert_user = $database->prepare( $cQuery )) {
                $cVerified = 0;
                $cPassword = password_hash($cPassword,PASSWORD_DEFAULT);
                $insert_user->bind_param('ssssiss', $cFirstName, $cLastName, $cPassword, $cInsertValidation, $cVerified, $cBirthDate, $cGender );
                $insert_user->execute();
                $insert_user->close();

                $cUserId = mysqli_insert_id( $database );
                ...
                if ($store_safety_code = $database->prepare('INSERT INTO safety_codes (user_id, code, requested, type) VALUES (?, ?, NOW(), ?)')) {
                    ...
                    $cLocation = "Location: mail_verify_information.php?cUserId=${cUserId}";
                    header($cLocation);
